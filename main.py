...
def get_settings_keyboard(current_settings: Dict) -> InlineKeyboardMarkup:
    ...
    notifications = current_settings.get('notifications_enabled', 1)
    ...
    keyboard = [
        ...
        [InlineKeyboardButton(f"🔔 Notifications: {'ON' if notifications else 'OFF'}", callback_data="toggle_notifications")],
        [InlineKeyboardButton("🔙 Main Menu", callback_data="main_menu")]
    ]
    return InlineKeyboardMarkup(keyboard)
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if data == "main_menu":
        ...
        settings = db_manager.get_user_settings(user_id)
        current_notif = settings.notifications_enabled if settings else 1
        new_notif = 0 if current_notif else 1
        
        db_manager.update_user_settings(user_id, notifications_enabled=new_notif)
        
        await query.answer(f"🔔 Notifikasi {'diaktifkan' if new_notif else 'dinonaktifkan'}!")
...
def main():
    ...
    import asyncio
    from notification_monitor import monitor_tokens
    bot = application.bot
    loop = asyncio.get_event_loop()
    loop.create_task(monitor_tokens(bot))
